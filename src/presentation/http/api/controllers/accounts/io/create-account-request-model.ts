...
// One compute provider bound to the account: the adapter (`provider`) plus the substrate it provisions
// (`platform` + `execution`), so an account can hold several providers routed by (platform, execution).
class ComputeModel {
    @IsString()
    provider: string;
