...
export type SlotAssignmentData = {
    id: string;
...
export type SlotAssignmentCreateParams = {
    environmentId: string;
    slotIndex: number;
    launch: WorkloadLaunch;
};

// One environment's seat on a pooled host. The slot index is the assignment's identity on the machine:
...
export class SlotAssignment {
    static create(params: SlotAssignmentCreateParams): SlotAssignment {
    ...
    static fromObject(data: SlotAssignmentData): SlotAssignment {
        return new SlotAssignment(data.id, data.environmentId, data.slotIndex, data.launch ?? {}, data.createdAt);
    }
