...
#
# The check-in is POST /internal/machineLeases/{id}:heartbeat (per-host bearer token): the body reports
...
# ---------------------------------------------------------------- agent mode
run_agent() {
    : "${SW_LEASE_ID:?SW_LEASE_ID is required}"
    : "${SW_LEASE_TOKEN:?SW_LEASE_TOKEN is required}"
    : "${SW_INTERNAL_URL:?SW_INTERNAL_URL is required}"

    interval="${SW_HEARTBEAT_INTERVAL_SECONDS:-3}"
    state_dir="${SW_STATE_DIR:-/tmp/sw-machine/${SW_LEASE_ID}}"
    mkdir -p "${state_dir}"
    ...
    heartbeat_url="${SW_INTERNAL_URL}/internal/machineLeases/${SW_LEASE_ID}:heartbeat"

    echo "[machine-agent ${SW_LEASE_ID}] agent up, reporting ${host_ip}, state in ${state_dir}"

    while true; do
        body="{\"hostIp\":\"${host_ip}\"}"
        response_file="${state_dir}/heartbeat.json"
        status=$(curl -s -o "${response_file}" -w "%{http_code}" -X POST \
            -H "Authorization: Bearer ${SW_LEASE_TOKEN}" -H "content-type: application/json" \
            -d "${body}" "${heartbeat_url}" || echo "000")

        if [ "${status}" = "404" ]; then
            # The machine is unknown to the control plane (returned/forgotten) — self-fence.
            echo "[machine-agent ${SW_LEASE_ID}] 404 from the control plane — stopping every slot and exiting"
        ...
        if [ "${status}" = "200" ]; then
            ...
            echo "[machine-agent ${SW_LEASE_ID}] check-in failed (${status}), retrying"
        fi
