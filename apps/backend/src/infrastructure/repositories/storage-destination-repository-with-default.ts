...
// destination reads the install default (per-project prefix keeps them apart). Writes pass through
// untouched — configuring a real destination still creates the project's own row, which then wins.
export class StorageDestinationRepositoryWithDefault extends StorageDestinationRepository {
    ...
    async find(projectId: ProjectId): Promise<StorageDestination | null> {
        return (await this.base.find(projectId))
            ?? StorageDestination.create({ bucket: defaultBucket, prefix: `projects/${projectId.getValue()}` });
    }
