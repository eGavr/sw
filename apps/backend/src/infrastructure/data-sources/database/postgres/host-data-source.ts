...
import { Host as HostEntity, HostData } from "../../../../domain/entities/host-pool/host";

import { ComputeHost } from "./typeorm/entities/host-pool/compute-host";
import { HostPlacement } from "./typeorm/entities/host-pool/host-placement";

@Injectable()
export class HostDataSource {
    constructor(private readonly dataSource: DataSource) {}

    async create(host: HostEntity): Promise<void> {
        const entity = ComputeHost.from(host);

        await this.dataSource.transaction(async (manager) => {
            const { placements, ...row } = entity;

            await manager.getRepository(ComputeHost).save(row);
    ...
    async findOne(id: string): Promise<HostData | null> {
        const host = await this.dataSource.getRepository(ComputeHost).findOne({ where: { id } });

        return host?.toObject() ?? null;
    }

    // The host holding this environment's seat, if any — the release path starts from the environment.
    async findByEnvironment(environmentId: string): Promise<HostData | null> {
    ...
    async withOne(id: string, apply: (data: HostData) => HostData): Promise<HostData | null> {
        return this.dataSource.transaction(async (manager) => {
            const locked = (await manager.query(
                "SELECT id FROM compute_host WHERE id = $1 FOR UPDATE",
                [id],
            )) as Array<{ id: string }>;
            ...
            const entity = await manager.getRepository(ComputeHost).findOneOrFail({ where: { id } });
    ...
    async withMostLoadedPlaceable(
        ...
        apply: (data: HostData) => HostData,
    ): Promise<HostData | null> {
        return this.dataSource.transaction(async (manager) => {
            const locked = (await manager.query(
                `SELECT id FROM compute_host
                 WHERE cloud_account_id = $1 AND binding_id = $2 AND state = ANY($3)
                   AND (SELECT count(*) FROM host_placement p WHERE p.host_id = compute_host.id) < capacity_slots
                 ORDER BY (SELECT count(*) FROM host_placement p WHERE p.host_id = compute_host.id) DESC, created_at
            ...
            const entity = await manager.getRepository(ComputeHost).findOneOrFail({ where: { id: locked[0].id } });
            const next = apply(entity.toObject());
    ...
    async delete(id: string): Promise<void> {
        await this.dataSource.getRepository(ComputeHost).delete({ id });
    }
    ...
    private async persist(manager: EntityManager, data: HostData): Promise<void> {
        await manager.getRepository(ComputeHost).update(data.id, {
            state: data.state,
