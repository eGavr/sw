...
// The per-account provisioning shape for the YC android adapters, read from a cloud account's opaque config
...
export type AndroidProvisioningOverrides = {
    folderId?: string;
    imageId?: string;
    zone?: string;
    subnetId?: string;
...
export function androidProvisioningOverrides(config: CloudConfig | undefined): AndroidProvisioningOverrides {
    if (!config) {
        return {};
    }
