...
**Дальше:** S2 — UI (Add cloud → Self-hosted, секция Machines, Attach machine с командой установки, cordon/drain/detach, подпись Machine у окружения); S3 — live на VM юзера (есть `/dev/kvm`); S4 — linux-слоты пула. Follow-ups: mTLS агента, ротация `machineToken`, реальная ёмкость машины vs оценка при провижне YC (после регистрации сервера аренда могла бы взять facts-ёмкость).
