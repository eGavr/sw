...
## Follow-up: VNC-труба переживает смерть сессии — capability-дыра, ПОЧИНИТЬ
...
1. **wd ws-прокси ре-валидирует установленные трубы** (главное): раз в ~10с `/status` ноды (механизм sw/alive — без команд в сессию) → сессия умерла → рвать соединение; новые подключения по мёртвому id отсекать до похода на ноду. Платформо-независимо, enforcement на нашей границе.
2. Пояс: **агент перезапускает x11vnc на грани busy→false** — рубит все трубы к дисплею в момент смерти сессии (supervisord поднимет заново). Дёшево, но платформа-специфично.
