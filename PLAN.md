...
На стенде internal (`:3002`) упал с unhandled `DOMException [TimeoutError]: The operation was aborted due to timeout` (стек из `abort_controller`), пока слот качал артефакт через CP (`HttpRemoteArtifactGateway`, fetch с `AbortSignal.timeout`, тело стримится в ответ). Таймаут, сработавший после начала стрима, вылетает как unhandled rejection и валит процесс — все агенты получают `000`, машины уходят в `offline`. Починить: ловить ошибки стрима (`pipeline` с обработкой), таймаут — на установление соединения/заголовки, не на весь стрим; в идеале — общий `unhandledRejection`-guard с логом вместо падения.
