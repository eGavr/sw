...
- **Осознанное смягчение секрет-модели:** сейчас capability знает только создатель; станет — «член проекта с нужным правом может восстановить id» (= и убить, и подключиться). Для командного продукта это фича (зависшую сессию прибивает коллега/админ; Grid UI так живёт), но **право выбрать аккуратно**: `sw.sessions.get` (viewer) — это read логов/видео, а live-id — управление → отдельное право (напр. `sw.sessions.getCurrent`/`sw.sessions.manage`) уровня developer+.
