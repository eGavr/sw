...
18. **Graceful teardown окружения с флашем логов — НЕ сделано (гонка: логи последней сессии теряются при быстром delete).**
    Кейс: сессия закончилась → агент ещё не отправил её логи (шлёт асинхронно на следующем тике heartbeat) → пользователь
    делает `DELETE env` → воркер `deprovision` = **`docker rm -f`** (SIGKILL, без грейса; в k8s SIGTERM идёт в PID 1 selenium,
    а агент — фоновый процесс) → контейнер+агент убиты до отправки → **логи последней сессии теряются**. Окно узкое (ограничено
    последней сессией), но потеря реальна. **Решение (выбрано, вариант 1): agent-coordinated graceful teardown через существующий
    heartbeat-канал.** `DELETE`→`deleting`; в **ответе heartbeat** сигнал агенту «тебя удаляют» → агент **дошлёт логи и сам
    погасится** (переиспользует `shutdown_environment` self-fence). Воркерский `deprovision` (force-rm) становится
