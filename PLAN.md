...
- ~~**Пагинация в UI: projects (сайдбар) и environments (таблица).**~~ — СДЕЛАНО (ветка `feat.ui-pagination`, PR влит). `lib/sw.ts` отдаёт `Page<T> = { items, nextPageToken }` (`listProjectsPage`/`listEnvironmentsPage`, единая точка `pageQuery`); оба списка на `useInfiniteQuery` + «Load more» (навбар-линк для проектов, футер-кнопка таблицы для окружений); 3s-поллинг окружений сохранён (react-query рефетчит все загруженные страницы → occupancy/state живые, freeing-effect и инвалидции работают поверх union'а). Проверено live против Keycloak: проекты 4 стр. по 1, окружения 2 стр. по 1 (pageSize=1), уникальны, токен завершается. `cloudAccounts` — без пагинации by design.
- **Дефолтная сортировка списков — newest-first (по `created_at` DESC), чтобы позже созданные env всплывали вверх** (запрос юзера 2026-08-31). Сейчас keyset фиксирован `ORDER BY created_at ASC, id ASC` (общий `keysetPage`, `page.ts`-codec, п.25) → новые окружения падают ВНИЗ (и вообще на последнюю страницу). Нужно развернуть в DESC. Подводные камни: (1) поменять и порядок, и направление keyset-сравнения курсора (`(created_at,id) < cursor` вместо `>`) — иначе токен сломается; (2) `keysetPage` — общий для projects и environments, решить: разворачивать для ОБОИХ (единообразно) или ввести направление как параметр и применить только к environments; (3) курсор `page.ts` кодирует `(created_at,id)` — направление в токене не хранится, оно из запроса, так что менять только helper/data-source, формат токена тот же; (4) проверить индексы (`environment(project_id,created_at,id)` работает и для DESC-скана — Postgres умеет обратный обход b-tree). Транспорт-контракт (`nextPageToken`) не меняется, фронт не трогаем.
...
## BYOC-трек (bring your own cloud): «пользователь подключает СВОЁ облако, платит за свои ресурсы, мы на них разворачиваем» — В РАБОТЕ (срез 1 машинерии влит)

Продуктовая модель (согласована): подключение облака = креды пользователя, окружения поднимаются в ЕГО фолдере за ЕГО счёт; «как именно развернём» — наше know-how, объявленное заранее (пользователь понимает цену).

**Решения юзера (2026-08-31):** секрет-стор в проде — **YC Lockbox** (готовый managed secrets-manager YC; наш контрол-плейн читает его под своей ambient YC-identity, содержимое = чужой SA-ключ; один стор cloud-agnostic — обслуживает креды для ЛЮБЫХ облаков юзера, per-cloud стор не нужен); **pluggable** по `SECRET_STORE=memory|fs|yc-lockbox` как `AUTH_STRATEGY`/`LOG_STORAGE`. Первый срез — **машинерию собрать и проверить локально на local/docker** (креды принимаются/сторятся/резолвятся, docker их не требует), YC IAM-обмен — следующий срез.

**СРЕЗ 1 — СДЕЛАНО (ветка `feat.byoc-cloud-credentials`):** порт-гейтвей `SecretStore` (`store/resolve/delete`) + реализации `InMemorySecretStore` (тест) / `FsSecretStore` (dev, `.dev-secrets/`, общий диск) + провайдер по `SECRET_STORE` (`yc-lockbox` пока кидает «not implemented»); VO `CloudCredential` (у границы гейтвея, НЕ в домене — чужой секрет; `reveal()` + редактирующие `toString/toJSON`). Connect: request-model получил опциональный `credential`, контроллер прокидывает, `CreateCloudAccountUseCase` инжектит `SecretStore` (сиблинг repo) → `store→ref` → `CloudAccount.create({credentialRef})` (overlap-проверка ДО store, чтобы конфликт не оставил orphan-секрет); presenter уже прячет ref. Provision: порт `EnvironmentProviderGateway.provision(env, cloudAccount, credential?)`, `PrepareNextEnvironmentUseCase` инжектит `SecretStore` → `resolve(credentialRef)→CloudCredential` → отдаёт в provision; docker/android-адаптеры credential игнорируют (ambient), gateways-сиблинги координирует use-case. UI: опциональное поле «Credentials» (textarea) в Connect-модалке `CloudsTab`, стейджится в `pendingConnect` и постится на Save. Гейты: be tsc/lint 0, +3 unit (редакция VO), +новые integration (connect стор+не-течёт+ref персистится; provision резолвит и отдаёт credential; null-путь). fe tsc 0.

Шаги дальше:
1. ~~**Креды при connect**~~ — СДЕЛАНО (срез 1, см. выше). Остаётся `yc-lockbox`-реализация `SecretStore` + `YandexComputeClient`: заменить ambient-metadata на IAM-обмен по SA-ключу юзера (`credential.reveal()`), `null`→ambient-фолбэк; каталог `requiresCredential(type)` (local=false, yandex-cloud=true) + энфорсмент в use-case (required-missing→400) + условный/required показ поля в UI; живой прогон на поднятом YC. = **СРЕЗ 2**.
...
5. **Каталог пер-инсталляцию**: `local` — ресурсы оператора, в SaaS-инсталляции его предлагать нельзя; симметрично (решение юзера 2026-08-31) **в локальной разработке не показывать `yandex-cloud`** — dev-инсталляция предлагает только `local`, прод — только реальные облака → набор типов в каталоге (`GET /v1/cloudTypes` и превью в Connect-модалке) становится конфигурацией инсталляции (напр. `CLOUD_CATALOG=local` / `CLOUD_CATALOG=yandex-cloud,...`).
