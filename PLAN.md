...
## [DESIGN] одна платформа — одно облако на проект: разрешить несколько, с порядком — НЕ начато (юзер, 2026-09-08)

Сейчас `CreateComputeBindingUseCase` отказывает во второй привязке той же пары (платформа, исполнение) по ВСЕМУ проекту:
запрос на окружение называет платформу и способ исполнения, но не облако, поэтому маршрут обязан быть однозначным.

**Зачем это в проде (обсуждение с юзером 2026-09-08).** Не «удобство для разработки», а три реальных сценария:
1. **Перелив под пик.** Свои машины — дешёвая база, публичное облако — пики. Вместо 429 в час пик система заказывает
   metal. Для тестового облака нагрузка пилообразная (утренний CI против ночи), так что это главный сценарий.
2. **Отказоустойчивость.** Провайдер лёг, грант отозвали, папку удалили — сегодня это «платформа не создаётся вообще».
   Со вторым облаком проект продолжает работать.
3. **Переезд без простоя.** Привязать оба, перестать сажать новое на старое (это `drain`, но на уровне платформы),
   дождаться, пока старое опустеет, отвязать.
Слабее, но тоже реальны: тарифная разница (массовый CI на своём железе, интерактив в облаке) и близость/комплаенс.

**Что уже готово:** окружение хранит `cloudAccountId`/`cloudType`/`computeKind` с момента создания, и
`RoutingEnvironmentProviderGateway` всю жизнь окружения ходит по этой записи. Значит переезд между облаками ему не
грозит, не хватает только ВЫБОРА в момент посадки.

**Предлагаемая форма:** упорядоченный список привязок платформы в проекте («основная + запасные»), а не свободный
набор. Запрос не меняется (платформа + исполнение), посадка идёт по списку: первая привязка, где есть место, забирает
окружение; 429 отдаётся, только когда мест нет НИГДЕ — гарантия сохраняется и становится честнее по смыслу.

Открытые вопросы: как задаётся порядок (поле на привязке или список на проекте); складываются ли квоты (скорее да:
потолок проекта = сумма привязок); что показывать в UI (в строке окружения — где оно приземлилось; в карточках — что
платформу делят и в каком порядке); нужен ли явный адрес цели в запросе (для демо и отладки — позже, отдельно);
health-based автопереключение (в первой версии НЕ делаем — только «нет места → следующая»).
