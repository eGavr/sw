...
3. **Прагматичный best-effort (дёшево, сейчас):** на delete-if-busy сперва DELETE сессии через ноду (агент ловит busy→false и с НОВЫМ фоновым пайплайном бэкграундит отгрузку), подождать ограниченный grace, потом `deprovision`. Не гарантирует, но с keyed+background-пайплайном почти всегда довезёт за копейки.
