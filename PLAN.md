...
## [DESIGN] одна платформа — одно облако на проект: как разрешить «и так, и так» — НЕ начато (юзер, 2026-09-08)

Сейчас `CreateComputeBindingUseCase` отказывает во второй привязке той же пары (платформа, исполнение) по ВСЕМУ проекту:
запрос на окружение называет платформу и способ исполнения, но не называет облако, поэтому маршрут обязан быть
однозначным. Следствие: `local` (наш docker-демон) и `self-hosted` (машины юзера) не могут одновременно обслуживать
`ubuntu/container` в одном проекте — переключение делается перепривязкой платформы (два вызова API).

Возврат старого byo-роута у `local` эту задачу НЕ решает и его не воскрешаем: это было второе имя для того же самого
(см. дизайн-док — `local` означает строго «docker-демон коробки, где живёт CP», без агента, токена и владельца-юзера).
Каталог, к слову, несколько видов на стереотип уже допускает (`yandex-cloud` + `ubuntu/container` = `vm` и `kubernetes`),
так что структурного запрета нет — проблема именно в неоднозначности маршрута.

Что нужно решить, если делаем:
1. **Как запрос называет цель.** Наименее магичный вариант — явный id привязки (или облака) в create-environment;
   альтернатива — политика предпочтения на проекте (упорядоченный список привязок).
2. **Что по умолчанию**, когда цель не названа: первая привязка, помеченная основной, или отказ с требованием назвать.
3. **Квоты.** `maxEnvironments` живёт на привязке; при двух привязках одной платформы считать их раздельно (тогда
   «потолок проекта» = сумма) и что показывать пользователю.
4. **Поведение при исчерпании.** Строгий 429 по названной привязке или перелив на вторую (последнее ломает
   предсказуемость и гарантию 429 — скорее нет).
5. **UI.** В форме нового окружения появляется выбор «где запустить», в карточках облаков — что платформу делят.
...
## [BUG] гарантия 429 не держится между привязками одного self-hosted облака — НЕ начато (замечено 2026-09-08)

Посадка места синхронна и упирается в headroom СВОЕГО пула (пул = аккаунт+привязка), а инвентарь машин общий на
аккаунт. Если на одном self-hosted облаке привязаны две платформы (`android/emulator` и `ubuntu/container`) и свободна
одна машина, оба пула независимо посадят по `enqueued`-аренде на неё же: оба create вернут 201, машину заберёт тот пул,
чей воркер провижнил первым, а второе окружение уйдёт в `failed` на провижне вместо синхронного 429. Проверено тестом:
пока машина УЖЕ занята арендой, второй платформе отказывают (`self-hosted-machine-pool.test`, «a machine held by one
platform's pool is not given to another platform»); дыра ровно в окне до провижна. Чинить: бюджет «аренд, ждущих
машину» считать на АККАУНТ (инвентарь), а не на пул — `maxAwaitingMachine` должен видеть `enqueued`-аренды всех
привязок аккаунта, тогда вторая посадка упрётся в 0 свободных машин и вернёт 429.
