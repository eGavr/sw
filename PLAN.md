...
## Follow-up (ВЫСОКИЙ приоритет, вскрыто аллокационным ретраем): агент отгружает логи/видео FOREGROUND и ломается при быстром переиспользовании env — ПОЧИНИТЬ
...
- **Направление фикса:** (1) на грани — синхронный СНИМОК: копия слайса лога в session-keyed файл + `ffmpeg q` (финализация своего keyed mp4), быстро, до записи новой сессии; (2) upload — в ФОН (detached), чтобы хартбиты не прерывались (нет reaper-reclaim, нет флапа); (3) файлы, keyed по session id (`sw-session-<id>.{log,mp4}`), чтобы старый upload и новая запись не коллизили; (4) `log_offset` фиксировать снимком границы на грани, а не выводить после медленной отгрузки.
