...
- **Confirm-диалог при удалении busy-окружения.** Удаление окружения с живой сессией — осознанный админский путь (deprovision убивает контейнер вместе с сессией), но сейчас мусорка на busy-строке срабатывает без подтверждения, и есть коварный side effect: **логи/видео сессии теряются** (агент отгружает их батчем на busy→false, а тут умирает вместе с контейнером — отгрузить некому). UI: подтверждение с честным текстом «живая сессия будет убита, её логи/видео не сохранятся». (Бэкенд-гард не нужен — семантика delete правильная.)
...
## Редизайн аллокации сессий: пессимистичная РЕЗЕРВАЦИЯ вместо optimistic pick+retry — В РАБОТЕ
...
## Sessions-таб: порядок табов + командный блок VNC — СЛЕДУЮЩИЙ ШАГ после редизайна аллокации

Спека юзера (2026-08-30), делать сразу после аллокационного редизайна:
- **Порядок табов: Logs | Video | VNC** (сначала логи, потом видео, потом vnc).
- **Активный таб зависит от пути входа:** переход со строки env (живая сессия → интерактив) → активен **VNC**; ручной ввод id в поле → активен **Logs**.
- **Действия с сессией — блок ПОД VNC-табом** (визуально отдельный блок, связанный с этим табом; Delete из шапки табов переезжает туда и активен только на VNC-табе). Команды:
  - **Delete session** (как сейчас, DELETE через `/api/wd`);
  - **Переход по URL**: поле «вставь URL» → под капотом обычный WD-трафик (`POST /session/{id}/url` через `/api/wd`).
- **Дизайн расширяемый:** команды как список дескрипторов (label + опц. input + mutation), чтобы дальше добавлять новые WD-команды без перевёрстки; пока ровно две.
