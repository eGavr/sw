...
## Follow-up: VNC-вид не замечает смерть сессии после kill — ПОЧИНИТЬ

Наблюдения юзера (2026-08-31):
1. **Full screen → Delete session**: вместо afterlife-вида (Session deleted + Logs|Video) юзер оказывается на голом VNC-экране с дефолтной заглушкой noVNC (iframe пережил смерть сессии). Разобраться, почему killed-ветка не отработала (или отработала не тем).
2. **Sessions-таб, VNC-вкладка только что закрытой сессии**: показывается окно VNC (мёртвый iframe с noVNC-заглушкой), а не текст «The session is not active…».
3. **Back из full screen возвращает на вкладку Logs, а надо на VNC** (пересмотр решения от 2026-08-31: `&view=logs` в Back-ссылке был ошибкой). Фикс: убрать `view=logs` из Back — liveness-дефолт сам делает правильно (жива → VNC, мертва/после Delete → Logs).

Вероятный общий корень: живость семплится ОДИН раз (проба/recovery при открытии, `staleTime: Infinity`) и не пересматривается после kill или внешней смерти сессии. Направление фикса: после onKilled локально переводить вид в «не активна» (и/или инвалидировать `sessionAlive`-query); для смертей извне — переоценка живости (лёгкий re-probe при возврате на VNC-вкладку или по ошибке iframe).

## Follow-up: при ЛОКАЛЬНОЙ разработке не пишутся ни логи, ни видео сессий — ПОЧИНИТЬ

Наблюдение юзера (2026-08-30, локальный стенд): сессии с `sw:logging`/`sw:video` не оставляют артефактов — вкладки Logs/Video всегда пустые. Вероятная причина: артефакты агент отгружает в internal, а тот кладёт их в **storage проекта** (`storageDestination` → S3) — у локального dev-проекта бакет не настроен, отгрузке некуда падать (и, похоже, падает молча). Починить локальный путь: варианты — (а) дефолтный dev-storage без настройки (локальный диск или in-memory с readback-ручками), (б) minio в dev-обвязке + авто-`storageDestination` при создании проекта в dev, (в) хотя бы честная ошибка/лог вместо тихой потери + подсказка в UI «configure storage in Settings». Заодно проверить: агент в local-docker образе вообще собирает логи/видео или это тоже часть проблемы.
