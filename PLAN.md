...
## [DESIGN] одна платформа на нескольких облаках проекта — СДЕЛАНО (2026-09-09)

Правило «одна привязка платформы на проект» снято: платформу может обслуживать несколько облаков, они пробуются в
порядке привязки — первое, где есть место, забирает окружение. Зачем (обсуждение с юзером): перелив под пик (свои
машины как база, публичное облако как запас — вместо 429 в час пик), отказоустойчивость (провайдер лёг — работаем на
втором), переезд без простоя (привязать оба, дождаться, пока старое опустеет, отвязать).

Сделано:
- домен: `CloudAccountList.candidatesFor(...)` отдаёт упорядоченный список размещений (порядок — время привязки,
  ничьи разбиваются по id), `pinnedTo(...)` — размещение, названное вызывающим; уникальность осталась внутри облака
  (`CloudAccount.bindCompute`), проектная снята;
- у привязки появилось время создания (миграция `1789600000000`) — это и есть порядок «первая привязанная — основная»;
- `CreateEnvironmentUseCase` обходит кандидатов: `RESOURCE_EXHAUSTED` (исчерпана квота ИЛИ нет мест в пуле) означает
  «спроси следующего», отказ доходит до вызывающего, только когда мест нет нигде; окружение, которое не удалось
  посадить, не остаётся в базе;
- **явная цель**: `computeBinding` в create-environment (uid или resource name) — пин обслуживается ровно названной
  привязкой и НЕ переливается; несуществующая/не обслуживающая платформу привязка → `INVALID_ARGUMENT`;
- ответ окружения несёт `cloudAccount`/`cloudType`/`computeKind` — видно, где приземлилось;
- UI: в форме нового окружения выбор «Run on» (появляется, когда платформу обслуживает больше одного облака), в строке
  окружения — машина или облако, в форме привязки платформа, занятая другим облаком, больше не блокируется, а
  подписывается «also on …».

Тесты: unit на порядок кандидатов и пин; integration — перелив на второе облако при полной машине, пин без перелива
(429 по названной цели), пин на чужую платформу → 400, и «второе облако можно привязать, второй раз на том же — нельзя».

Живая проверка на маке (2026-09-09): в `alice-demo` `ubuntu/container` привязан к ОБОИМ облакам — self-hosted
(привязан раньше, машина мака) и `local` (docker). Окружение без указания цели ушло на self-hosted и поднялось слотом
агента (`sw-slot-628cd7b9…`), окружение с `computeBinding` привязки `local` — контейнером самого контрол-плейна; оба
одновременно ACTIVE на одной платформе в одном проекте.
...
`sw:cloudType` в запросе сессии был начат и откачен: он добавлял вторую точку выбора там, где выбор уже сделан, и
сужал пул на ровном месте. Пин на уровне окружения (`computeBinding`) — единственное место, где называют облако.

**Осталось follow-up:** явный `priority` у привязки (сейчас порядок = время привязки, переставить можно только
перепривязкой) и подсказка в UI, в каком порядке облака будут пробоваться.

## [FLAKE] интеграционный прогон: `Parse Error: Expected HTTP/, RTSP/ or ICE/` — НЕ начато (наблюдается с 2026-09-07)

В полном прогоне 1–4 теста из ~300 падают с `Parse Error: Expected HTTP/, RTSP/ or ICE/` от supertest, каждый раз
РАЗНЫЕ и в разных наборах (наблюдалось в projects/IAM, sessions, environments, cloud-types); повтор того же набора в
одиночку всегда зелёный. Значит это не поведение кода, а транспорт теста: supertest получает мусор вместо HTTP-ответа —
похоже на переиспользование keep-alive соединения, которое сервер уже закрыл (агент по умолчанию + `app.close()` между
кейсами). Чинить в тестовой обвязке: свой `http.Agent` с `keepAlive: false` (или `Connection: close`) в
`TestingApp`/хелперах запроса. Пока живём с ним: перед выводом «сломалось» прогоняем набор отдельно.

## [NAMING] один концепт — два имени: `Stereotype` и `Substrate` — НЕ начато (юзер, 2026-09-08; решение слова за юзером)
...
api под nodemon, потом прогон `cloud-types.test.integration`; между вспышками 20 из 20 соединений с теми же кредами
проходят, а в логе самого `sw-db` отказов аутентификации НЕТ — то есть отвергает не сервер, а что-то на пути
(подозрение на проброс порта Lima; причина вспышки вне нашего кода и остаётся неизвестной).
