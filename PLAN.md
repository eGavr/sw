...
- **Доступ (решение юзера — НЕ смягчать секрет-модель):** восстановить id может **только создатель СЕССИИ** (не окружения! — в pool-модели на «твоём» env может крутиться чужая сессия, и env-creator получил бы контроль над ней). Так как сессии не персистятся, для правила нужны **ownership-метаданные сессии БЕЗ секрета**: таблица `(environment_id, created_by, created_at)` — владение не секрет, capability по-прежнему живёт только на ноде; строка чистится вместе с концом сессии. Чужим — **404** (не 403: не палим существование сессии). Админский кейс «прибить чужое зависшее» уже покрыт удалением окружения (`environments.delete` → deprovision убивает сессию) — смягчение не нужно.
