...
- **S4 (сделано, ветка `feat.self-hosted-any-stereotype`)** — машина отдаёт любой поддерживаемый стереотип, не только
  `android/emulator`. `launch` получил вид слота (`kind: emulator|container`); контейнерный слот — тот же linux-нод,
  что у всех остальных backend-ов (`linuxNodeProvisioning` + `agentBootstrap`), только `docker run` делает агент на
  машине пользователя (порт слота публикуется на порт нода, loopback-адрес CP переписывается в `host.docker.internal`,
  контейнер снимается вместе со слотом). В каталоге у `self-hosted` появился `ubuntu/container` на `baremetal`;
  `PATCH …/machines/{m}` меняет `provides` уже подключённой машины (без переустановки агента); условие
  `VncStackMissing` сужено до эмуляторных стереотипов. **Ограничения модели, зафиксированы:** машина держится ровно
  одной арендой, поэтому слоты на ней в один момент одного вида (закреплено тестом; но пока машина ещё не захвачена
  воркером, два пула одного аккаунта успевают посадить по аренде на неё же — синхронный 429 в этом окне не срабатывает,
  см. баг в PLAN); платформа привязывается к одному облаку на проект
  (`ubuntu/container` на self-hosted требует снять её с `local`); две machine-строки на одну коробку невозможны —
  порты слотов совпадут.
