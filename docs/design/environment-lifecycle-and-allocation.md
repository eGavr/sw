...
- **Self-fence агента (сирота):** если бэк на хартбит ответил **404** — про окружение он больше не знает
  (reap по ложному протуханию, или упавший deprovision), значит контейнер осиротел. Агент сам гасит
  окружение — шлёт SIGINT в PID 1 (supervisord, как это делает и сам базовый образ), а `docker run --rm`
  удаляет контейнер. **Никакой reconcile-логики на бэке не заводим** — источник правды остаётся БД, а сироту
  закрывает сам контейнер. Любой ДРУГОЙ сбой (сеть/5xx/409) агент повторяет, чтобы кратковременный блип бэка
  не снёс живые окружения (404 ≠ «бэк недоступен»).
