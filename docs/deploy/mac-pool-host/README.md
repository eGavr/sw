...
- **Видео сессий** (`sw:video`) пишет **scrcpy с устройства** (`--record`, поток самого девайса, без
  дисплея) — на маке нужен `brew install scrcpy` (тот же adb, что у SDK, на PATH агента). Нет scrcpy —
  видео молча пропускается (best effort), окружение не страдает.
...
(проверка `command -v`): **`scrcpy` ≥ 2.2** (Android 14 и `--time-limit` для записи видео;
дистрибутивный 1.25 не годится — брать
...
`5900+i`, websockify на `127.0.0.1:7900+i`; наружу — только wd-порт слота. Видео сессий слот пишет
тем же `scrcpy` с устройства (`SW_VIDEO_RECORDER=scrcpy`, второй инстанс рядом с зеркалом; jar
сервера scrcpy удаляет с устройства сразу после старта, версии не конфликтуют), `perl` для запуска
рекордера — в базовой системе есть.
